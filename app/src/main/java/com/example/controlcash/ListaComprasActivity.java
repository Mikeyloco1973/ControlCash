...
public class ListaComprasActivity extends AppCompatActivity {
    ...
    private void eliminarCompra(int id) {
        ...
        if (result > 0) {
            ...
            Toast.makeText(this, "No se pudo eliminar la compra", Toast.LENGTH_SHORT).show();
        }
    }
    ...
    private void configurarSwipeParaEliminar() {
        ItemTouchHelper.SimpleCallback swipeCallback = new ItemTouchHelper.SimpleCallback(0, ItemTouchHelper.LEFT) {
            ...
            public void onSwiped(@NonNull RecyclerView.ViewHolder viewHolder, int direction) {
                ...
                if (compra.usuario != null && compra.usuario.equals(usuario)) {
                    eliminarCompra(compra.id);
                } else {
                    Toast.makeText(ListaComprasActivity.this, "Solo puedes eliminar tus propias compras", Toast.LENGTH_SHORT).show();
            ...
            public void onChildDraw(@NonNull Canvas c, @NonNull RecyclerView recyclerView,
                ...
                int position = viewHolder.getAdapterPosition();
                if (position < 0 || position >= listaCompras.size()) return;

                Compra compra = listaCompras.get(position);
                View itemView = viewHolder.itemView;
                Paint paint = new Paint();

                boolean esPropia = compra.usuario != null && compra.usuario.equals(usuario);
                paint.setColor(esPropia ? Color.RED : Color.GRAY);
